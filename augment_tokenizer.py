...
def augment_tokenizer(in_tok_path, 
    ...
    config = {
        ...
        'num_base_tokens': len(mp.pieces) - len(new_tokens)
    }
...
def test_augmented_tokenizer(out_tok_dir):
    ...
    inserted_tokens = [sp_model.IdToPiece(id) for id in range(config['insertion_index'], config['insertion_index'] + config['num_new_tokens'])]
