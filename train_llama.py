...
def setup_logs(dataset: str, local_rank: int, world_size: int):
    if local_rank > 0:
        sys.stdout = open(os.devnull, 'w')

    if local_rank == 0:
        wandb.init(project="funcllama", name=f"{dataset}-{world_size}-load-test")
        # wandb.init(project="opt", name=save_name)

def get_train_test(input_file: str, dataset: str) -> Tuple[list, list]:
...
def load(ckpt_dir: str, tokenizer_path: str, local_rank: int, world_size: int, dataset: str) -> FunctionLM:
    
    start_time = time.time()
    
    # -----------> load tokenizer <----------
    tokenizer = Tokenizer(model_path=tokenizer_path)
    print(f"Loaded tokenizer with {tokenizer.n_words} tokens.")
    # -----------> load LLama model <----------
    ...
    # -------> Load function toolkens and idxs {func_tooken: idx, ...} <-------
    func_dict_path = f"../data/{dataset}/func_dict.json"
    func_dict = json.load(open(func_dict_path, "r"))
    
    # --------> Augmented LLM: combine base model and func_dict <----- Where the combination happens
    funcmodel = FunctionLM(model, tokenizer, func_dict = func_dict)
    print(f"Loaded in {time.time() - start_time:.2f} seconds")
    
    return funcmodel

def train_epoch(funcmodel: FunctionLM, trainset: list, optimizer: torch.optim.Optimizer, local_rank: int, only_functoken: bool, log_each: bool = False, func_list: list = None):
    ...
    for case_idx, prompt in tqdm(enumerate(trainset)):
        ...
        optimizer.zero_grad()
        loss, result = funcmodel.get_loss([prompt], only_functoken=only_functoken) # backprop one example at a time ?
        loss.backward()
        optimizer.step()
...
def validation_epoch(funcmodel: FunctionLM, testset: list, local_rank: int, log_each: bool = False, func_list: list = None):
...
def save_checkpoint(funcmodel: FunctionLM, epoch: int, log_prefix: str, dataset: str):
...
def main(ckpt_dir: str, tokenizer_path: str, input_file: str = None, lr: float = 1e-3, num_epochs: int = 20, dataset: str = "gsm8k-xl", log_prefix="", only_functoken=False, log_each=False):
    ...
    setup_logs(dataset, local_rank, world_size)

    # ------- load model artifcats -------
    
    # augmented model
    funcmodel = load(ckpt_dir, tokenizer_path, local_rank, world_size, dataset=dataset)#func_dict=func_dict)
    
    trainset, testset = get_train_test(input_file, dataset)
    ...
    for epoch in range(num_epochs):
        ...
        funcmodel = train_epoch(funcmodel, trainset, optimizer, local_rank, only_functoken, log_each, func_list)
        
        validation_epoch(funcmodel, testset, local_rank, log_each, func_list)

        save_checkpoint(funcmodel, epoch, log_prefix, dataset)
