...
class FunctionLM(nn.Module):
    ...
    def get_loss(self, raw_inputs, only_functoken=False):
        ...
        with torch.no_grad():
            ...
            token_logits = self.model.base_output(h) # (bsz, seqlen, vocab_size)
            # print(h.device)
        
        func_logits = self.model.augmented_output(h) # (h.float()) # (bsz, seqlen, len(func_list))
    ...
    def generate(
        ...
        for cur_pos in range(start_pos, total_len):
            ...
            logits_token = self.model.base_output(h[:, -1, :]).float() # (bsz, vocab_size)
            logits_func = self.model.augmented_output(h[:, -1, :]) #(h[:, -1, :].float()) # (bsz, len(func_list))
