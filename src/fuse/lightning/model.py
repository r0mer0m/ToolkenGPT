...
class PLModel(LightningModule):
    ...
    def __init__(self, model: nn.Module, tokenizer, rank:int, config: Dict[str, Union[str, int]]):
        ...
        self.stopping_criteria = StoppingCriteriaList([
            StoppingCriteriaSub(stops = [self._tokenizer.eoc_token_id])
        ])
    ...
    def on_train_batch_end(self, outputs, batch, batch_idx: int) -> None:
        ...
        if (batch_idx + 1) % 20 == 0:
            self._log_trigger_metrics(stage="train")
            self.results = defaultdict(list)
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        self._log_trigger_metrics(stage="validation")         
    ...
    def _log_trigger_metrics(self, stage="train"):
